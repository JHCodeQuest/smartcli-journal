...
import datetime
import os
from cmdjournal.database import add_entry, view_entries, add_tag, get_entries_by_tag
import sqlite3
...
def open_journal():
    #move into the directory where created file is
    os.chdir(os.getcwd())
...
def add_content():
    ...
    cursor.execute(
        ...
        (content, str(datetime.datetime.now()), emotion_label, subjectivity_label, dream_meaning)
    )
    connection.commit()
...
def controls():
    ...
    #allow the user to pick another function
    choice = input("Do you need anything else? (y/n)")
    if choice == "y" or choice == "yes":
        controls()
    #if not exit the journal
    else:
        print("See you later!")
        close_connection()
